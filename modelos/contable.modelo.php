...
class ModeloContable{
	...
	static public function mdlCargarArchivo($tabla,$data){
		...
		if($data['planilla'] == 'paihuen'){
			...
			$stmt = Conexion::conectar()->prepare("INSERT INTO $tabla(archivo,libro,periodo,activos,agricultura,ganaderia,resto,prestamos,tarjetas,mutuales,proveedores,seguros,deudaBancaria,bienesDeCambio,cajaBancos,pasivoCorriente,pasivoTotal,patrimonioNeto,ingresoBrutoMensual,cargasSocialesReales,sueldos,honorarios) VALUES(:archivo,:libro,:periodo,:activos,:agricultura,:ganaderia,:resto,:prestamos,:tarjetas,:mutuales,:proveedores,:seguros,:deudaBancaria,:bienesDeCambio,:cajaBancos,:pasivoCorriente,:pasivoTotal,:patrimonioNeto,:ingresoBrutoMensual,:cargasSocialesReales,:sueldos,:honorarios)");
